...
shuffle!(unseen)
levels = ["LH", "EH", "CO"]
train, test = partition(labelled, 0.6; rng=12, stratify=labelled.label)
...
begin
    ...
    self_tuning_tree = TunedModel(
            model=model_new,
            tuning=AdaptiveParticleSwarm(n_particles=30),
...
for (n, i) in enumerate(2:4)
    hist!(a, eva[:, i], scale_to=-0.6, offset=n, direction=:x, normalization=:probability)
end
f